...
**运行时不做客户端生成题目**：`play/[level]` 嵌入池内前 45 道（`SETS_PER_LEVEL`，与图解页 No.1〜45 对应，`?n=` 直达），「別の問題」池内循环；打印页按档动态 import 题库 JSON 随机抽样。`generateByLevel` 只用于离线脚本——它未命中目标难度时返回最接近档（hard 实测命中 0/5），不能拿给用户当指定难度用。
