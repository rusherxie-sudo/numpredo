...
export const VARIANTS: Variant[] = [
  ...
  },
  {
    ...
    faq: [
      ...
      { q: '6×6数独はどこで遊べますか？', a: '当サイトでは6×6はルール解説のみで、プレイできるのは9×9の数独です。考え方は同じなので、まずは<a href="/play/beginner/">9×9の初級</a>で基本に慣れてから挑戦するのがおすすめです。' },
    ],
  },
  {
    slug: 'inequality',
  ...
  },
  {
    ...
    sections: [
      ...
      { h: 'このページでの遊び方', body: ['上の盤面でそのままプレイできます。<strong>薄い朱色のマスが対角線</strong>です。行・列・ブロックだけでなく、対角線上で数字が重複したときも即座にハイライトされるので、「斜めの見落とし」に気づきながら慣れていけます。', 'メモ（候補の書き込み）・ヒント・元に戻す・一時停止は通常の<a href="/play/beginner/">難易度別プレイ</a>と同じ操作です。数字を確定させると、同じ行・列・ブロックに加えて<strong>対角線上のメモも自動で消えます</strong>。', '収録は全30問。やさしい順に並んでいて、「別の問題」ボタンで次の問題に進めます。'] },
    ...
    faq: [
      ...
      { q: '対角線数独はどこで遊べますか？', a: 'このページでそのままプレイできます（無料・登録不要）。収録している30問はすべて、対角線ルールのもとで「答えが一つだけ」「推測なしの論理だけで解ける」ことを検証済みです。通常の数独は<a href="/play/beginner/">難易度別プレイ</a>へ。' },
