...
export const VARIANTS: Variant[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { h: '通常の数独との違い', body: ['見た目は同じ9×9ですが、対角線の制約がある分、<strong>少ないヒントでも唯一解になる</strong>ように作られています。通常の数独のつもりで対角線を見落とすと、途中で行き詰まってしまいます。', '逆に、対角線を「もう2本の単元」として最初から意識すれば、通常の数独にはない絞り込みができ、慣れると爽快に解けます。'] },
    ],
    faq: [
      ...
      { q: '対角線数独はどこで遊べますか？', a: '当サイトでは対角線数独（X数独）はルール解説のページで、プレイできるのは通常の9×9数独です。基本テクニックは共通なので、まず<a href="/play/intermediate/">中級</a>や<a href="/play/advanced/">上級</a>で消去法に慣れるのがおすすめです。' },
    ],
  },
];
