...
for (const rel of listPages()) {
  if (rel.includes('[')) {
    ...
    if (rel === 'play/[level]/[n].astro') {
      ...
      // 30 = SETS_PER_LEVEL（同步点：play/[level]/[n].astro、gen-sitemap-lastmod.ts；曾因硬编码 12 漏掉 90 页的 datePublished）
      if (d) for (const lv of slugsFromData('src/data/levels.ts')) for (let nn = 1; nn <= 30; nn++) map[`/play/${lv}/${nn}/`] = d;
      continue;
    }
